...
function getIconForCondition(condition: string): keyof typeof iconMap {
  const lowerCondition = condition.toLowerCase();
  if (lowerCondition.includes("mobile")) return "smartphone";
  if (lowerCondition.includes("desktop")) return "monitor";
  if (lowerCondition.includes("bot")) return "globe";
  return "globe";
}
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            {snapshot.user_agent && (
              <div className="bg-secondary/50 rounded p-2 col-span-2">
                ...
                <p className="font-medium text-foreground mt-0.5 truncate">{snapshot.user_agent}</p>
              </div>
            )}
            {snapshot.ip_geo && (
              <div className="bg-secondary/50 rounded p-2">
                <span className="text-muted-foreground">Geo:</span>
                <p className="font-medium text-foreground mt-0.5">{snapshot.ip_geo}</p>
              </div>
            )}
            {snapshot.referer && (
              <div className="bg-secondary/50 rounded p-2">
                <span className="text-muted-foreground">Referer:</span>
                <p className="font-medium text-foreground mt-0.5 truncate">{snapshot.referer}</p>
              </div>
            )}
          </div>
          ...
          {snapshot.content_preview && (
            <div className="rounded-md bg-secondary/50 p-3 text-xs text-muted-foreground leading-relaxed">
              <span className="text-foreground/70 font-medium">Content Preview:</span>
              <p className="mt-1">{snapshot.content_preview}</p>
            </div>
          )}

          {/* Redirect Chain */}
          {snapshot.redirect_chain && snapshot.redirect_chain.length > 0 && (
            <div className="rounded-md bg-secondary/50 p-3">
              <span className="text-xs text-foreground/70 font-medium">Redirect Chain:</span>
              <div className="mt-1 space-y-1">
                {snapshot.redirect_chain.map((url: string, i: number) => (
